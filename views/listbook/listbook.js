(async function () {
    ...
    const table = document.getElementById('libritos')
    ...
    document.addEventListener("keyup", e => {
        if (e.target.matches("#search")) {
            ...
            libros.forEach(libro => {
                ...
                if (titulo.indexOf(texto) !== -1 || autor.indexOf(texto) !== -1) {
                    table.innerHTML += template(libro);
                }
            })
        }
        botonesActualizarInfoEliminar();
    })
    ...
    function botonesActualizarInfoEliminar() {
